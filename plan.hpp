...
public:
	...
	Plan(float3 A,float3 N,Color couleur,std::vector<double> materiau):point(A),normale(N) ,Objet(couleur,materiau){
		assert(N.length()>0.00001);//normale non nulle
	}
	...
	virtual double Intersection(Rayon myRay) {
		...
		if(d<0.0001)return -1;//le plan est derriere l'origine du rayon
		return d;
	}
