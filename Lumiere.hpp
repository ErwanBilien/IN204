...
public:
	...
	bool Ombre(std::vector<std::shared_ptr<Objet>> listeObjets,float3 M){
		return false;
	}
...
public:
	...
	bool Ombre(std::vector<std::shared_ptr<Objet>> listeObjets,float3 M){//renvoie true si la lumiere projete une ombre sur le point M
...
Color CalculLuminosite(
	...
	float3 M,float3 normale,float3 vectObservation,
	double brillance)
{
